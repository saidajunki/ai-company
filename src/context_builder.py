...
def build_system_prompt(
    ...
    **_unused,
    ...
    sections: list[str] = [
        ...
        "継続中の事業には常に関心を持つ。停滞を検知したら『原因 / ボール保有者 / 次アクション / 期限』を明示して再始動する。",
        "長期間触れられていない事業は掘り起こし、継続・停止・ピボットを必ず判断する。方針や予算変更が必要なときのみCreatorへ<consult>する。",
        "各事業のコスト推移を監視し、費用対効果が悪化したら縮小・停止・再設計の案を出す。",
