...
def _build_infra_guidance_section() -> str:
    ...
    return "\n".join([
        ...
        "- Traefikが自動でSSL証明書を取得する（httpChallenge方式）",
        "- 証明書取得にはドメインのDNSがVPSのIP(162.43.23.107)を指している必要がある",
        "- 新ドメインは `*.app.babl.tech` のワイルドカードDNSで自動解決される",
        "- コンテナが正常起動していないと証明書取得が失敗する（ACMEチャレンジエラー）",
        "- デプロイ後にSSLエラーが出る場合: コンテナが起動しているか確認 → `docker compose logs` でエラー確認",
        "- 証明書の状態確認: `cat /opt/apps/traefik/acme.json | python3 -c \"import json,sys; data=json.load(sys.stdin); certs=data.get('letsencrypt',{}).get('Certificates',[]); print([c['domain']['main'] for c in certs])\"`",
        "- Traefikログ確認: `docker logs traefik --tail=50 2>&1 | grep -i acme`",
        "",
        "### 注意事項",
        ...
        "- WordPressのdbコンテナはapps-networkに接続不要（内部通信のみ）",
        "- dbコンテナにTraefikラベルを付けないこと",
    ])
...
def _build_format_section() -> str:
    return "\n".join([
        ...
        "",
        "<consult>",
        "Creatorに相談したい内容（重大事項のみ: 方向性/ビジョン変更、課金・契約・広告出稿などお金が動く、法/規約/炎上/個人情報など高リスク）。",
        "質問 + 選択肢 + 推奨 + 影響 + 上限コスト を簡潔に書く。",
        "</consult>",
        "",
        "<plan>",
